...
def S37():
    ...
    if ("iar" == "iar" or "iar" == ""):
        ...
        GCC_TOOLCHAIN_PATH = shortPath(r"file:/C:/Program%20Files%20(x86)/IAR%20Systems/Embedded%20Workbench%208.3/".replace(
            "file:", "").replace("%20", " "))
...
def OTA():
    if not '"%PROJECT_DIR%\..\\..\\protocol\\zigbee\\tool\\image-builder\\image-builder-windows.exe" --create "%TARGET_BPATH%.ota" --version 101 --manuf-id 0x1219 --image-type 29 --tag-id 0x0000 --tag-file "%TARGET_BPATH%.gbl" --string "EBL HiddenSingleFireSwitch_Z3SE_SDK6630"':
        sys.exit(0)
    print (" ")
    ...
    IMAGE_BUILDER = '"%PROJECT_DIR%\..\\..\\protocol\\zigbee\\tool\\image-builder\\image-builder-windows.exe" --create "%TARGET_BPATH%.ota" --version 101 --manuf-id 0x1219 --image-type 29 --tag-id 0x0000 --tag-file "%TARGET_BPATH%.gbl" --string "EBL HiddenSingleFireSwitch_Z3SE_SDK6630"'.replace("\t", "\\t")
