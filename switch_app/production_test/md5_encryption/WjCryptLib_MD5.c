...
//  
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static void CommonMemCopy(void* dest, const void *source, uint32_t bytes)
{
...
static void CommonMemSet(void *dest, uint8_t val, uint32_t bytes)
{
  uint8_t *d = (uint8_t *)dest;
			   ...
			   )
{
    ...
    if( used )
    {
        ...
        if( BufferSize < free )
        {
            CommonMemCopy( &Context->buffer[used], Buffer, BufferSize );
            return;
        }

        CommonMemCopy( &Context->buffer[used], Buffer, free );
    ...
    CommonMemCopy( Context->buffer, Buffer, BufferSize );
}
				...
				)
{
    ...
    if(free < 8)
    {
        CommonMemSet( &Context->buffer[used], 0, free );
        TransformFunction( Context, Context->buffer, 64 );
        used = 0;
        free = 64;
    }

    CommonMemSet( &Context->buffer[used], 0, free - 8 );
